...
#define AFPVOL_MSWINDOWS                                                       \
  (1 << 9)                         /* deal with ms-windows yuckiness.          \
                                      this is going away. */
...
#define AFPVOL_NOFILEID                                                        \
  (1 << 15) /* don't advertise createid resolveid and deleteid calls */
...
#define vol_unix_priv(vol)                                                     \
  (afp_version >= 30 && ((vol)->v_flags & AFPVOL_UNIX_PRIV))
