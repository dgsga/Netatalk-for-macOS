...
#define ADEID_NAME                                                             \
  3 /* Note: starting with Netatalk 2.1 we do NOT alway set the name */
...
#define AD_HEADER_LEN                                                          \
  (ADEDLEN_MAGIC + ADEDLEN_VERSION + ADEDLEN_FILLER + ADEDLEN_NENTRIES)
...
#define AD_DATASZ1                                                             \
  (AD_HEADER_LEN + ADEDLEN_NAME + ADEDLEN_COMMENT + ADEDLEN_FILEI +            \
   ADEDLEN_FINDERI + (ADEID_NUM_V1 * AD_ENTRY_LEN))
...
#define AD_NEWSZ2                                                              \
  (ADEDLEN_DID + ADEDLEN_AFPFILEI + ADEDLEN_SHORTNAME + ADEDLEN_PRODOSFILEI +  \
   ADEDLEN_PRIVDEV + ADEDLEN_PRIVINO + ADEDLEN_PRIVSYN + ADEDLEN_PRIVID)

/* 725 */
#define AD_DATASZ2                                                             \
  (AD_DATASZ1 + AD_NEWSZ2 + ((ADEID_NUM_V2 - ADEID_NUM_V1) * AD_ENTRY_LEN))
...
#define ADFLAGS_OPENFORKS                                                      \
  (1 << 7) /* check for open fork in ad_metadata function */
...
extern ssize_t ad_read(struct adouble *, const u_int32_t, const off_t, char *,
                       const size_t);
extern ssize_t ad_pread(struct ad_fd *, void *, size_t, off_t);
extern ssize_t ad_write(struct adouble *, const u_int32_t, off_t, const int,
...
/* ad_mmap.c */
extern void *ad_mmapread(struct adouble *, const u_int32_t, const off_t,
                         const size_t);
extern void *ad_mmapwrite(struct adouble *, const u_int32_t, const off_t,
...
extern int ad_setid(struct adouble *, const dev_t dev, const ino_t ino,
                    const u_int32_t, const u_int32_t, const void *);
extern u_int32_t ad_getid(struct adouble *, const dev_t, const ino_t,
                          const cnid_t, const void *);
...
#ifdef WITH_SENDFILE
extern int ad_readfile_init(const struct adouble *ad, const int eid, off_t *off,
                            const int end);
#endif
