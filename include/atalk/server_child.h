...
typedef struct server_child_data {
  ...
  int disasociated; /* 1 if this is not a child, but a child from a previous
                       afpd master */
...
extern void server_child_kill_one_by_id(server_child *children,
                                        const int forkid, const pid_t pid,
                                        const uid_t, const u_int32_t len,
                                        char *id, u_int32_t boottime);
extern int server_child_transfer_session(server_child *children, int forkid,
                                         pid_t, uid_t, int, uint16_t);
extern void server_child_setup(server_child *, const int,
                               void (*)(const pid_t));
