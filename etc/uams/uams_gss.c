...
static void log_status(char *s, OM_uint32 major_status,
  ...
  while (1) {
    ...
    LOG(log_info, logtype_uams, "uams_gss.c :do_gss_auth: %s %.*s (error %s)",
        s, (int)msg.length, msg.value, strerror(errno));
...
static int get_afpd_principal(void *obj, gss_name_t *server_name) {
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_KRB5SERVICE, (void *)&service,
                           &servicelen) < 0)
    return 1;
  ...
  if (!service || !servicelen || !fqdn || !fqdnlen) {
    LOG(log_note, logtype_uams,
        "get_afpd_principal: could not retrieve information from afpd, using "
        "default service principal(s)");
  ...
  LOG(log_debug, logtype_uams, "get_afpd_principal: importing principal `%s'",
      principal);
  major_status = gss_import_name(&minor_status, &s_princ_buffer,
...
static int get_client_username(char *username, int ulen,
  ...
  if (namelen >= ulen) {
    ...
    LOG(log_error, logtype_uams, "get_client_username: username `%s' too long",
        client_name_buffer.value);
    ret = 1;
  } else {
...
static int wrap_sessionkey(gss_ctx_id_t context, struct session_info *sinfo) {
  ...
  gss_wrap(&status, context, 1, GSS_C_QOP_DEFAULT, &sesskey_buff, NULL,
           &wrap_buff);
...
static int acquire_credentials(gss_name_t *server_name,
  ...
  if ((envp = getenv("KRB5_KTNAME")))
    ...
    LOG(log_debug, logtype_uams,
        "acquire credentials: acquiring credentials (uid = %d) - $KRB5_KTNAME "
        "not found in env",
        (int)geteuid());
   ...
   * We don't care about the mechanisms or about the time for which they are
   * valid.
   */
...
static int accept_sec_context(gss_ctx_id_t *context, gss_cred_id_t server_creds,
  ...
  major_status = gss_accept_sec_context(
      &minor_status, context, server_creds, ticket_buffer,
      GSS_C_NO_CHANNEL_BINDINGS, client_name, NULL, authenticator_buff,
      &ret_flags, NULL, NULL);
...
/* return 0 on success */
static int do_gss_auth(void *obj, char *ibuf, int ticket_len, char *rbuf,
                       int *rbuflen, char *username, int ulen,
...
static int gss_logincont(void *obj, struct passwd **uam_pwd, char *ibuf,
   ...
   * username (format unspecified) padded, when necessary, to end on an even
   * boundary ticket length (u_int16_t) ticket
   */
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_SESSIONINFO, (void *)&sinfo, NULL) <
      0)
    return AFPERR_MISC;
  ...
  if (ibuflen < 4) {
    LOG(log_info, logtype_uams, "uams_gss.c :LoginCont: user is %s, no ticket",
        p);
    return AFPERR_PARAM;
  }
  ...
  if (ticket_len > ibuflen) {
    LOG(log_info, logtype_uams,
        "uams_gss.c :LoginCont: invalid ticket length (%u > %u)", ticket_len,
        ibuflen);
    return AFPERR_PARAM;
  }
...
int uam_setup(const char *path) {
  if (uam_register(UAM_SERVER_LOGIN_EXT, path, "Client Krb v2", gss_login,
                   gss_logincont, gss_logout, gss_login_ext) < 0)
    if (uam_register(UAM_SERVER_LOGIN, path, "Client Krb v2", gss_login,
