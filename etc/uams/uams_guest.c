...
static int noauth_login(void *obj, struct passwd **uam_pwd, char *ibuf _U_,
                        size_t ibuflen _U_, char *rbuf _U_, size_t *rbuflen) {
  struct passwd *pwent;
  char *guest, *username;
  ...
  *rbuflen = 0;
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_GUEST, (void *)&guest, NULL) < 0)
    return AFPERR_MISC;

  if (uam_afpserver_option(obj, UAM_OPTION_USERNAME, (void *)&username, NULL) <
      0)
    return AFPERR_MISC;

  strcpy(username, guest);
  if ((pwent = getpwnam(guest)) == NULL) {
    LOG(log_error, logtype_uams, "noauth_login: getpwnam( %s ): %s", guest,
        strerror(errno));
...
static int noauth_printer(char *start, char *stop, char *username,
  ...
  if (getpwnam(username) == NULL) {
    LOG(log_info, logtype_uams, "Bad Login NoAuthUAM: %s: %s", username,
        strerror(errno));
    return (-1);
  }
...
static int uam_setup(const char *path) {
  if (uam_register(UAM_SERVER_LOGIN_EXT, path, "No User Authent", noauth_login,
                   NULL, NULL, noauth_login_ext) < 0)
    return -1;

  if (uam_register(UAM_SERVER_PRINTAUTH, path, "NoAuthUAM", noauth_printer) < 0)
    return -1;
