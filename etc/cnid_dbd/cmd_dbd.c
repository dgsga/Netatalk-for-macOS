...
/*
  ...
  OK If -f is requested, ensure -e is too.
     Check if volumes is using AFPVOL_CACHE, then wipe db from disk. Rebuild
  from ad-files.
  ...
  OK F/D  try: read CNID from ad file (if cnid caching is on)
          ...
          -> on mismatch: use CNID from file, update database (deleting both
  found CNIDs first)
  ...
  St Spec
  -- ----
  OK Step through dbd (the one on disk) and rebuild-db from pass 1 and delete
  any CNID from dbd not in rebuild db. This in only done in exclusive mode.
*/
...
static void usage(void) {
  printf(
      "dbd (Netatalk %s)\n"
      "Usage: dbd [-e|-t|-v|-x] -d [-i] | -s [-c|-n]| -r [-c|-f] | -u <path to "
      "netatalk volume>\n"
      ...
      "      7. Check for orphaned CNIDs in database (requires -e)\n"
      "      8. Open and close adouble files\n"
      "      Options: -c Don't create .AppleDouble stuff, only cleanup "
      "orphaned.\n"
      "               -f wipe database and rebuild from IDs stored in "
      "AppleDouble\n"
      "                  files, only available for volumes without "
      "'nocnidcache'\n"
      "                  option. Implies -e.\n\n"
      "   -u Upgrade:\n"
      ...
      "the CNID must of course be synched to them files first with a plain -r "
      "rebuild!\n",
      VERSION);
}
...
int main(int argc, char **argv) {
  int c;
  int ret = -1;
  int dump = 0, scan = 0, rebuild = 0, prep_upgrade = 0, rebuildindexes = 0,
      dumpindexes = 0;
  dbd_flags_t flags = 0;
  char *volpath;
  int cdir;
  ...
  if (loadvolinfo(volpath, &volinfo) == -1) {
    dbd_log(LOGSTD,
            "Not a Netatalk volume at '%s', no .volinfo file at "
            "'%s/.AppleDesktop/.volinfo' or unknown volume options",
            volpath, volpath);
    exit(EXIT_FAILURE);
  }
  ...
  if (stat(volinfo.v_dbpath, &st) != 0) {
    if (errno != ENOENT) {
      dbd_log(LOGSTD, "Can't stat dbpath \"%s\": %s", volinfo.v_dbpath,
              strerror(errno));
      exit(EXIT_FAILURE);
    }
    if ((mkdir(volinfo.v_dbpath, 0755)) != 0) {
      dbd_log(LOGSTD, "Can't create dbpath \"%s\": %s", dbpath,
              strerror(errno));
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (dbdirfd == -1 && errno == ENOENT) {
    if (errno == ENOENT) {
      if ((mkdir(dbpath, 0755)) != 0) {
        dbd_log(LOGSTD, "Can't create .AppleDB for \"%s\": %s", dbpath,
                strerror(errno));
        exit(EXIT_FAILURE);
      }
    } else {
  ...
  */
  if (!nocniddb) {
    ...
    if (dbif_env_open(dbd, &db_param,
                      (db_locked == LOCK_EXCL) ? (DBOPTIONS | DB_RECOVER)
                                               : DBOPTIONS) < 0) {
