...
static struct server *test_usockfn(struct volinfo *volinfo) {
  ...
  for (i = 0; i < maxvol; i++) {
    if ((srv[i].volinfo) &&
        (strcmp(srv[i].volinfo->v_path, volinfo->v_path) == 0)) {
      return &srv[i];
    }
  }
  return NULL;
    ...
    if (up->count > 0) {
      ...
      if (t < (up->tm + TESTTIME)) {
        ...
        if (up->count > MAXSPAWN) {
          ...
          return -1; /* just exit, dont sleep, because we might have work to do
                        for another client  */
        }
      } else {
...
/* ------------------ */
static void catch_child(int sig _U_) { sigchild = 1; }
  ...
  while (1) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
      ...
      if (WIFSIGNALED(status)) {
        LOG(log_info, logtype_cnid, "cnid_dbd[%i] received signal %i", pid,
            WTERMSIG(status));
      }
      sigchild = 0;
    }
    ...
    if (actual_len != len) {
      LOG(log_error, logtype_cnid, "error/short read (dir): %s",
          strerror(errno));
      goto loop_end;
    }
    volpath[len] = '\0';
    ...
    if ((volinfo = allocvolinfo(volpath)) == NULL) {
      LOG(log_severe, logtype_cnid, "allocvolinfo(\"%s\"): %s", volpath,
          strerror(errno));
      goto loop_end;
    }
