...
static char *utompath(char *upath) {
  ...
  /* convert charsets */
  if ((size_t)-1 ==
      (outlen = convert_charset(myvolinfo->v_volcharset, CH_UTF8_MAC,
...
static char *mtoupath(char *mpath) {
  ...
  if ((size_t)-1 ==
      (outlen = convert_charset(CH_UTF8_MAC, myvolinfo->v_volcharset,
                                myvolinfo->v_maccharset, m, inplen, u, outlen,
                                &flags))) {
...
/*
  Check for wrong encoding e.g. "." at the beginning is not CAP encoded (:2e)
  although volume is default !AFPVOL_USEDOTS. We do it by roundtripiping from
  volcharset to UTF8-MAC and back and then compare the result.
*/
  ...
  if ((adpar_ok = access(myvolinfo->ad_path(".", ADFLAGS_DIR), F_OK)) != 0) {
    if (errno != ENOENT) {
      dbd_log(LOGSTD, "Access error on '%s/%s': %s", cwdbuf,
              myvolinfo->ad_path(".", ADFLAGS_DIR), strerror(errno));
      return -1;
    }
...
static cnid_t check_cnid(const char *name, cnid_t did, struct stat *st,
  ...
  if (ad_cnid && db_cnid && (ad_cnid == db_cnid)) {
    ...
    dbd_log(LOGSTD, "CNID mismatch for '%s/%s', db: %u, ad-file: %u", cwdbuf,
            name, ntohl(db_cnid), ntohl(ad_cnid));
    if (!(dbd_flags & DBD_FLAGS_SCAN)) {
      dbd_log(
          LOGSTD,
    ...
    if (!(dbd_flags & DBD_FLAGS_SCAN)) {
      /* Ensure the cnid from the ad-file is not already occupied by another
       * file */
      ...
      if (rply.result == CNID_DBD_RES_OK) {
        ...
        if (dbif_txn_close(dbd, ret) != 0)
          return CNID_INVALID;
        db_cnid = rply.cnid;
        dbd_log(LOGSTD, "New CNID for '%s/%s': %u", cwdbuf, name,
                ntohl(db_cnid));
  ...
  while ((ep = readdir(dp))) {
    ...
    **************************************************************************/
    ...
    if (-1 == (encoding_ok = check_name_encoding(ep->d_name))) {
      ...
      /* For dirs we could try to get a CNID for it and recurse, but currently I
       * prefer not to */
      continue;
    }
    ...
    **************************************************************************/
    if (S_ISDIR(st.st_mode) &&
        (cnid || nocniddb)) { /* If we have no cnid for it we cant recur */
      strcat(cwdbuf, "/");
...
int cmd_dbd_scanvol(DBD *dbd_ref, struct volinfo *vi, dbd_flags_t flags) {
  ...
  if (vi->v_volcharset != CH_UTF8) {
    dbd_log(LOGSTD, "Not a Unicode volume: %s, %u != %u", vi->v_volcodepage,
            vi->v_volcharset, CH_UTF8);
    return -1;
  }
  ...
  /* temporary rebuild db, used with -re rebuild to delete unused CNIDs, not
   * used with -f */
  if (!nocniddb && (flags & DBD_FLAGS_EXCL) && !(flags & DBD_FLAGS_FORCE)) {
    ...
    if (dbif_env_open(dbd_rebuild, &db_param,
...
exit:
  if (!nocniddb) {
    ...
    if ((ret == 0) && dbd_rebuild && (flags & DBD_FLAGS_EXCL) &&
      ...
      /* We can only do this in exclusive mode, otherwise we might delete CNIDs
         added from other clients in between our pass 1 and 2 */
