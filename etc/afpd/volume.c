 ...
 */
static char *volxlate(AFPObj *obj, char *dest, size_t destlen, char *src,
...
static void volset(struct vol_option *options, struct vol_option *save,
  ...
  if (optionok(tmp, "allow:", val)) {
    ...
    LOG(log_error, logtype_afpd,
        "The old codepage system has been removed. Please make sure to read "
        "the documentation !!!!");
    ...
    LOG(log_debug, logtype_afpd, "CNID Server for volume '%s': %s:%s", volname,
        val + 1, p ? p + 1 : Cnid_port);
...
/* ------------------------------- */
static int creatvol(AFPObj *obj, struct passwd *pwd, char *path, char *name,
  ...
  LOG(log_maxdebug, logtype_afpd,
      "createvol: Volume '%s' -> UTF8-MAC Name: '%s'", name, tmpname);
  ...
  LOG(log_maxdebug, logtype_afpd, "createvol: Volume '%s' ->  Longname: '%s'",
      name, tmpname);
  ...
  for (volume = Volumes; volume; volume = volume->v_next) {
    if ((utf8_encoding() &&
        ...
        (!utf8_encoding() &&
         (strcasecmp_w(volume->v_macname, mactmpname) == 0))) {
      if (volume->v_deleted) {
        volume->v_new = hide = 1;
      } else {
        LOG(log_error, logtype_afpd,
            "Duplicate volume name, check AppleVolumes files: previous: "
            "\"%s\", new: \"%s\"",
  ...
  if (options) {
    ...
    if (options[VOLOPT_DBPATH].c_value)
      volume->v_dbpath =
          volxlate(obj, NULL, MAXPATHLEN, options[VOLOPT_DBPATH].c_value, pwd,
                   path, name);
    ...
    if (!user) {
      if (options[VOLOPT_PREEXEC].c_value)
        volume->v_preexec =
            volxlate(obj, NULL, MAXPATHLEN, options[VOLOPT_PREEXEC].c_value,
                     pwd, path, name);
      volume->v_preexec_close = options[VOLOPT_PREEXEC].i_value;

      if (options[VOLOPT_POSTEXEC].c_value)
        volume->v_postexec =
            volxlate(obj, NULL, MAXPATHLEN, options[VOLOPT_POSTEXEC].c_value,
                     pwd, path, name);

      if (options[VOLOPT_ROOTPREEXEC].c_value)
        volume->v_root_preexec =
            volxlate(obj, NULL, MAXPATHLEN, options[VOLOPT_ROOTPREEXEC].c_value,
                     pwd, path, name);
      ...
      if (options[VOLOPT_ROOTPOSTEXEC].c_value)
        volume->v_root_postexec =
            volxlate(obj, NULL, MAXPATHLEN,
                     options[VOLOPT_ROOTPOSTEXEC].c_value, pwd, path, name);
    }
  }
...
static int hostaccessvol(int type, const char *volname, const char *args,
    ...
    if (compare_ip((struct sockaddr *)&client, ai->ai_addr) == 0) {
      if (type == VOLOPT_DENIED_HOSTS)
        LOG(log_info, logtype_afpd,
            "AFP access denied for client IP '%s' to volume '%s' by denied "
            "list",
  ...
  if (type == VOLOPT_ALLOWED_HOSTS)
    LOG(log_info, logtype_afpd,
        "AFP access denied for client IP '%s' to volume '%s', not in allowed "
        "list",
...
static int readvolfile(AFPObj *obj, struct afp_volume_name *p1, char *p2,
  ...
  while (1) {
    if ((read_lock(fd, 0, SEEK_SET, 0)) != 0) {
      ...
      if (!retries) {
        LOG(log_error, logtype_afpd,
            "readvolfile: can't lock volume file \"%s\"", path);
        if (fclose(fp) != 0) {
  ...
  while (myfgets(buf, sizeof(buf), fp) != NULL) {
    ...
    case '/':
      ...
      *volname = '\0';
      ...
      /* check allow/deny lists (if not afpd master loading volumes for Zeroconf
         reg.): allow -> either no list (-1), or in list (1) deny -> either no
         list (-1), or not in list (0) */
      if (parent_or_child == 0 ||
          (accessvol(options[VOLOPT_ALLOW].c_value, obj->username) &&
           (accessvol(options[VOLOPT_DENY].c_value, obj->username) < 1) &&
           hostaccessvol(VOLOPT_ALLOWED_HOSTS, volname,
                         options[VOLOPT_ALLOWED_HOSTS].c_value, obj) &&
           (hostaccessvol(VOLOPT_DENIED_HOSTS, volname,
                          options[VOLOPT_DENIED_HOSTS].c_value, obj) < 1))) {
        ...
        if (volxlate(obj, tmp, sizeof(tmp) - 1, volname, pwent, path, NULL) ==
            NULL)
          continue;
...
static int getvolspace(struct vol *vol, u_int32_t *bfree, u_int32_t *btotal,
  ...
  maxsize = (vol->v_flags & AFPVOL_A2VOL)
                ? 0x01fffe00
...
static int getvolparams(u_int16_t bitmap, struct vol *vol, struct stat *st,
  ...
  if ((bitmap &
    ...
    if (getvolspace(vol, &bfree, &btotal, &xbfree, &xbtotal, &bsize) !=
        AFP_OK) {
      if (isad) {
...
static int stat_vol(u_int16_t bitmap, struct vol *vol, char *rbuf,
  ...
  if ((ret = getvolparams(bitmap, vol, &st, rbuf + sizeof(bitmap), &buflen)) !=
      AFP_OK) {
    *rbuflen = 0;
    return (ret);
  }
...
void load_volumes(AFPObj *obj) {
  ...
  if ((*obj->username == '\0') || (obj->options.flags & OPTION_NOUSERVOL)) {
    ...
    if (readvolfile(obj, &obj->options.uservol, "AppleVolumes", 1, pwent) < 0 &&
        readvolfile(obj, &obj->options.uservol, ".AppleVolumes", 1, pwent) <
            0 &&
...
static int volume_openDB(struct vol *volume) {
  ...
  if (volume->v_cnidscheme == NULL) {
    ...
    LOG(log_info, logtype_afpd, "Volume %s use CNID scheme %s.", volume->v_path,
        volume->v_cnidscheme);
  }
  ...
  volume->v_cdb =
      cnid_open(volume->v_path, volume->v_umask, volume->v_cnidscheme, flags,
                volume->v_cnidserver ? volume->v_cnidserver : Cnid_srv,
                volume->v_cnidport ? volume->v_cnidport : Cnid_port);
  ...
  if (!volume->v_cdb && !(flags & CNID_FLAG_MEMORY)) {
    ...
    flags |= CNID_FLAG_MEMORY;
    volume->v_cdb =
        cnid_open(volume->v_path, volume->v_umask, "tdb", flags, NULL, NULL);
#ifdef SERVERTEXT
...
static void check_ea_sys_support(struct vol *vol) {
  ...
  if (vol->v_vfs_ea == AFPVOL_EA_AUTO) {
    ...
    if (process_uid)
      if (seteuid(0) == -1) {
        LOG(log_error, logtype_afpd,
            "check_ea_sys_support: can't seteuid(0): %s", strerror(errno));
        exit(EXITERR_SYS);
      }
    ...
    if ((sys_setxattr(vol->v_path, eaname, eacontent, 4, 0)) == 0) {
      ...
      LOG(log_warning, logtype_afpd,
          "volume \"%s\" does not support Extended Attributes, using ea:ad "
          "instead",
          vol->v_localname);
...
int afp_openvol(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (NULL == getcwd(path, MAXPATHLEN)) {
    ...
    LOG(log_error, logtype_afpd, "afp_openvol(%s): volume pathlen too long",
        volume->v_path);
    return AFPERR_MISC;
  }
  ...
  if (ret == AFP_OK) {
    ...
    savevolinfo(volume, volume->v_cnidserver ? volume->v_cnidserver : Cnid_srv,
                volume->v_cnidport ? volume->v_cnidport : Cnid_port);
...
int afp_setvolparams(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_,
  ...
  *rbuflen = 0;
  ...
  ad_init(&ad, vol->v_adouble, vol->v_ad_options);
  if (ad_open(vol->v_path, ADFLAGS_HF | ADFLAGS_DIR, O_RDWR, 0666, &ad) < 0) {
    if (errno == EROFS)
      return AFPERR_VLOCK;
...
static int create_special_folder(const struct vol *vol,
  ...
  if ((ret = stat(p, &st)) < 0) {
    if (folder->precreate) {
      if (ad_mkdir(p, folder->mode)) {
        LOG(log_debug, logtype_afpd, "Creating '%s' failed in %s: %s", p,
            vol->v_path, strerror(errno));
        free(p);
        free(q);
        return -1;
  ...
  if (!ret && folder->hide) {
    ...
    if (ad_entry(&ad, ADEID_FINDERI)) {
      memcpy(&attr, ad_entry(&ad, ADEID_FINDERI) + FINDERINFO_FRFLAGOFF,
             sizeof(attr));
      attr |= htons(FINDERINFO_INVISIBLE);
      memcpy(ad_entry(&ad, ADEID_FINDERI) + FINDERINFO_FRFLAGOFF, &attr,
             sizeof(attr));
    }
...
const struct vol *getvolumes(void) { return Volumes; }
