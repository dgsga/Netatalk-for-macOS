...
static void ipc_reconnect_handler(int sig _U_) {
  ...
  if (ipc_child_write(AFPobj->ipc_fd, IPC_GETSESSION,
                      AFPobj->sinfo.clientid_len,
...
static void afp_dsi_reload(int sig _U_) { reload_request = 1; }
...
static void afp_dsi_debug(int sig _U_) { debug_request = 1; }
    ...
    case DSIFUNC_CMD:
      ...
      if (replaycache[rc_idx].DSIreqID == dsi->clientID &&
        ...
        if (afp_switch[function]) {
          ...
          err =
              (*afp_switch[function])(obj, (char *)&dsi->commands, dsi->cmdlen,
