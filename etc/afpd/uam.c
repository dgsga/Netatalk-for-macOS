...
struct uam_mod *uam_load(const char *path, const char *name) {
  ...
  if (!mod->uam_fcn->uam_setup || ((*mod->uam_fcn->uam_setup)(name) < 0)) {
...
struct passwd *uam_getname(void *private, char *name, const int len) {
  ...
  if (obj->options.ntdomain && obj->options.ntseparator) {
    ...
    size_t ulen = strlen(obj->options.ntdomain) +
    ...
    if ((p = malloc(ulen + 1))) {
      ...
      if (pwent) {
        ...
        if (len < MAXUSERLEN) {
          ...
          LOG(log_error, logtype_uams, "MAJOR:The name %s is longer than %d",
  ...
  if ((size_t)-1 ==
      (namelen = convert_string((utf8_encoding()) ? CH_UTF8_MAC
...
#ifdef TRU64
void uam_afp_getcmdline(int *ac, char ***av) { afp_get_cmdline(ac, av); }
 ...
 */
{
  ...
  if ((rc = sia_ses_init(&entity, argc, argv, hostname, username, tty, colinput,
                         gssapi)) != SIASUCCESS) {
