...
int afp_openfork(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_, char *rbuf,
  ...
  */
  ...
  path = s_path->m_name;
  if ((ofork = of_alloc(vol, curdir, path, &ofrefnum, eid, adsame, st)) ==
      NULL) {
    return (AFPERR_NFILE);
  }
  ...
  if (access & OPENACC_WR) {
    ...
    if (ad_open(upath, adflags, O_RDONLY, 0, ofork->of_ad) < 0) {
      ...
      case ENOENT:
         ...
         * We need to check AFPFORK_OPEN in afp_closefork(). eg fork open
         * read-only then create in open read-write.
      ...
      default:
        LOG(log_error, logtype_afpd,
            "afp_openfork('%s/%s'): ad_open: errno: %i (%s)", getcwdpath,
            s_path->m_name, errno, strerror(errno));
        goto openfork_err;
        break;
      }
    } else {
...
static int byte_lock(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_,
  ...
  *rbuflen = 0;
  ...
  if (NULL == (ofork = of_find(ofrefnum))) {
    LOG(log_error, logtype_afpd,
        "afp_bytelock: of_find(%d) could not locate fork", ofrefnum);
    return (AFPERR_PARAM);
  }
...
static int crlf(struct ofork *of) {
  ...
  if (ad_meta_fileno(of->of_ad) == -1 ||
      !memcmp(ufinderi, ad_entry(of->of_ad, ADEID_FINDERI), 8)) { /* META */
...
static ssize_t read_file(struct ofork *ofork, int eid, off_t offset,
                         u_char nlmask, u_char nlchar, char *rbuf,
 ...
 */
static int read_fork(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf,
  ...
  LOG(log_debug, logtype_afpd,
      "afp_read(name: \"%s\", offset: %jd, reqcount: %jd)", of_name(ofork),
      (intmax_t)offset, (intmax_t)reqcount);

  savereqcount = reqcount;
  saveoff = offset;
  if (ad_tmplock(ofork->of_ad, eid, ADLOCK_RD, saveoff, savereqcount,
                 ofork->of_refnum) < 0) {
    err = AFPERR_LOCK;
    goto afp_read_err;
  }
  ...
  *rbuflen = MIN(reqcount, *rbuflen);
  LOG(log_debug, logtype_afpd,
      "afp_read(name: \"%s\", offset: %jd, reqcount: %jd): reading %jd bytes "
      "from file",
...
int afp_closefork(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_,
  ...
  *rbuflen = 0;
  ...
  if (of_closefork(ofork) < 0) {
    LOG(log_error, logtype_afpd, "afp_closefork(%s): of_closefork: %s",
        of_name(ofork), strerror(errno));
    return (AFPERR_PARAM);
  }
...
static ssize_t write_file(struct ofork *ofork, int eid, off_t offset,
                          char *rbuf, size_t rbuflen, const int xlate) {
  char *p, *q;
  ssize_t cc;
...
int afp_getforkparams(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_,
  ...
  *rbuflen = 0;
  ...
  if (AFP_OK !=
      (ret = getforkparams(ofork, bitmap, rbuf + sizeof(u_short), &buflen))) {
    return (ret);
  }
