 ...
 * */
static ssize_t build_fce_packet(struct fce_packet *packet, char *path, int mode,
                                uint32_t event_id) {
  size_t pathlen = 0;
  ssize_t data_len = 0;
  uint64_t *t;
  ...
  /* This should never happen, but before we bust this server, we send nonsense,
   * fce listener has to cope */
  if (pathlen >= MAXPATHLEN)
    pathlen = MAXPATHLEN - 1;

  packet->datalen = pathlen;

  /* This is the payload len. Means: the packet has len bytes more until packet
   * is finished */
...
static void send_fce_event(char *path, int mode) {
  ...
  for (int i = 0; i < udp_sockets; i++) {
    ...
    if (sent_data != data_len) {
      ...
      LOG(log_error, logtype_afpd,
          "send_fce_event: error sending packet to %s:%s, transfered %d of %d: "
          "%s",
...
static int register_fce(const char *u_name, int is_dir, int mode) {
  ...
  if (mode == FCE_TM_SIZE) {
    ...
    if (strlen(cwd) + strlen(u_name) + 1 >= MAXPATHLEN) {
      LOG(log_error, logtype_afpd, "FCE file name too long: %s/%s", cwd,
          u_name);
      return AFPERR_PARAM;
    }
  ...
  if (!(mode & FCE_TM_SIZE) &&
    ...
    LOG(log_debug9, logtype_afpd, "Coalesced fc event <%d> for <%s>", mode,
        full_path_buffer);
    return AFP_OK;
  }

  LOG(log_debug9, logtype_afpd, "Detected fc event <%d> for <%s>", mode,
      full_path_buffer);
...
static void check_saved_close_events(int fmodwait) {
  ...
  if (last_close_event.time && ((last_close_event.time + fmodwait) < now)) {
    LOG(log_debug, logtype_afpd, "check_saved_close_events: sending event: %s",
...
int fce_register_file_modification(struct ofork *ofork) {
  ...
  if (NULL == (u_name = mtoupath(vol, of_name(ofork), ofork->of_did,
                                 utf8_encoding()))) {
    return AFPERR_MISC;
  }
...
void shortsleep(unsigned int us) { usleep(us); }
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "d:e:h:p:P:s:")) != -1) {
    ...
    case '?':
      fprintf(stdout,
              "%s: [ -p Port -h Listener1 [ -h Listener2 ...] -P path -s "
              "Delay_between_events_in_us -e event_code -d Duration ]\n",
              argv[0]);
      exit(1);
      break;
    case 'd':
