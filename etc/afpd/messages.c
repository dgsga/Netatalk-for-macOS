...
void readmessage(AFPObj *obj) {
    ...
    maxmsgsize = (obj->proto == AFPPROTO_DSI) ? MIN(MAX(((DSI *) obj->handle)->attn_quantum, MAXMESGSIZE), MAXPATHLEN)
                                              : MAXMESGSIZE;
...
int afp_getsrvrmesg(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf, size_t *rbuflen) {
    ...
    *rbuflen += sizeof(bitmap);
    ...
    if (msglen) {
        if ((size_t) -1 ==
            (outlen = convert_string(obj->options.unixcharset, utf8 ? CH_UTF8_MAC : obj->options.maccharset,
