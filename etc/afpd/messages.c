...
void readmessage(AFPObj *obj) {
  ...
  maxmsgsize = (obj->proto == AFPPROTO_DSI)
                   ? MIN(MAX(((DSI *)obj->handle)->attn_quantum, MAXMESGSIZE),
                         MAXPATHLEN)
                   : MAXMESGSIZE;
  ...
  if (message != NULL) {
    ...
    if (seteuid(euid) < 0) {
      LOG(log_error, logtype_afpd, "Could not switch back to uid %d: %s", euid,
          strerror(errno));
      exit(EXITERR_SYS);
    }
...
int afp_getsrvrmesg(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  msgsize = (obj->proto == AFPPROTO_DSI)
                ? MAX(((DSI *)obj->handle)->attn_quantum, MAXMESGSIZE)
                : MAXMESGSIZE;
