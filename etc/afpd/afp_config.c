...
#ifdef USE_SRVLOC
static void SRVLOC_callback(SLPHandle hslp _U_, SLPError errcode,
                            void *cookie) {
...
static char *srvloc_encode(const struct afp_options *options,
                           const char *name) {
  static char buf[512];
  char *conv_name;
  unsigned char *p;
  ...
  if ((size_t)-1 ==
      (convert_string_allocate(options->unixcharset, options->maccharset, name,
                               -1, &conv_name)))
    return (char *)name;
...
static void dsi_cleanup(const AFPConfig *config) {
  ...
  err = SLPDereg(hslp, dsi->srvloc_url, SRVLOC_callback, &callbackerr);
  if (err != SLP_OK) {
    LOG(log_error, logtype_afpd,
        "dsi_cleanup: Error unregistering %s from SRVLOC", dsi->srvloc_url);
    goto srvloc_dereg_err;
  }

  if (callbackerr != SLP_OK) {
    LOG(log_error, logtype_afpd,
        "dsi_cleanup: Error in callback while trying to unregister %s from "
        "SRVLOC (%d)",
        dsi->srvloc_url, callbackerr);
    goto srvloc_dereg_err;
  }
...
static afp_child_t *dsi_start(AFPConfig *config, AFPConfig *configs,
  ...
  if (!(child = dsi_getsession(dsi, server_children,
...
static AFPConfig *DSIConfigInit(const struct afp_options *options,
  ...
  if ((config = (AFPConfig *)calloc(1, sizeof(AFPConfig))) == NULL) {
    LOG(log_error, logtype_afpd, "DSIConfigInit: malloc(config): %s",
        strerror(errno));
    return NULL;
  }

  LOG(log_debug, logtype_afpd, "DSIConfigInit: hostname: %s, ip/port: %s/%s, ",
      options->hostname, options->ipaddr ? options->ipaddr : "default",
      options->port ? options->port : "548");

  if ((dsi = dsi_init(protocol, "afpd", options->hostname, options->ipaddr,
                      options->port, options->flags & OPTION_PROXY,
  ...
  if (!(options->flags & OPTION_NOSLP)) {
    ...
    if (err != SLP_OK) {
      LOG(log_error, logtype_afpd,
          "DSIConfigInit: Error opening SRVLOC handle");
      goto srvloc_reg_err;
    }
    ...
    srvloc_hostname = srvloc_encode(
        options, (options->server ? options->server : options->hostname));

    if ((p) || afp_port == 548) {
      l = snprintf(dsi->srvloc_url, sizeof(dsi->srvloc_url),
                   "afp://%s/?NAME=%s", hostname, srvloc_hostname);
    } else {
      l = snprintf(dsi->srvloc_url, sizeof(dsi->srvloc_url),
                   "afp://%s:%d/?NAME=%s", hostname, afp_port, srvloc_hostname);
    }

    if (l == -1 || l >= (int)sizeof(dsi->srvloc_url)) {
      LOG(log_error, logtype_afpd,
          "DSIConfigInit: Hostname is too long for SRVLOC");
      dsi->srvloc_url[0] = '\0';
      goto srvloc_reg_err;
    }

    err = SLPReg(hslp, dsi->srvloc_url, SLP_LIFETIME_MAXIMUM, "afp", "",
                 SLP_TRUE, SRVLOC_callback, &callbackerr);
    if (err != SLP_OK) {
      LOG(log_error, logtype_afpd,
          "DSIConfigInit: Error registering %s with SRVLOC", dsi->srvloc_url);
      dsi->srvloc_url[0] = '\0';
      goto srvloc_reg_err;
    }
    ...
    if (callbackerr != SLP_OK) {
      LOG(log_error, logtype_afpd,
          "DSIConfigInit: Error in callback trying to register %s with SRVLOC",
          dsi->srvloc_url);
      dsi->srvloc_url[0] = '\0';
      goto srvloc_reg_err;
    }

    LOG(log_info, logtype_afpd, "Sucessfully registered %s with SRVLOC",
        dsi->srvloc_url);
...
static AFPConfig *AFPConfigInit(struct afp_options *options,
  ...
  if ((refcount = (unsigned char *)calloc(1, sizeof(unsigned char))) == NULL) {
    LOG(log_error, logtype_afpd, "AFPConfigInit: calloc(refcount): %s",
        strerror(errno));
    return NULL;
  }
