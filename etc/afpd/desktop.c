    ...
    switch (obj->proto) {
        case AFPPROTO_DSI: {
            ...
            while ((iovcnt = dsi_write(dsi, rbuf, buflen))) {
                if ((cc = write(si.sdt_fd, rbuf, iovcnt)) < 0) {
                    LOG(log_error, logtype_afpd, "afp_addicon(%s): write: %s", icon_dtfile(vol, fcreator),
                        strerror(errno));
                    dsi_writeflush(dsi);
                    return AFPERR_PARAM;
                }
...
char *mtoupath(const struct vol *vol, char *mpath, cnid_t did, int utf8) {
    ...
    if ((size_t) -1 ==
        (outlen = convert_charset((utf8) ? CH_UTF8_MAC : vol->v_maccharset, vol->v_volcharset, vol->v_maccharset, m,
...
char *utompath(const struct vol *vol, char *upath, cnid_t id, int utf8) {
    ...
    /* convert charsets */
    if ((size_t) -1 ==
        (outlen = convert_charset(vol->v_volcharset, (utf8) ? CH_UTF8_MAC : vol->v_maccharset, vol->v_maccharset, u,
                                  outlen, mpath, MAXPATHLEN, &flags))) {
        LOG(log_error, logtype_afpd, "Conversion from %s to %s for %s (%u) failed.", vol->v_volcodepage,
            vol->v_maccodepage, u, ntohl(id));
        goto utompath_error;
    }
