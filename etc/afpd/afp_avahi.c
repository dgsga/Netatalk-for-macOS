...
static void register_stuff(void) {
  ...
  if (!ctx->group) {
    if (!(ctx->group =
              avahi_entry_group_new(ctx->client, publish_reply, ctx))) {
  ...
  if (avahi_entry_group_is_empty(ctx->group)) {
    ...
    for (volume = getvolumes(); volume; volume = volume->v_next) {
      ...
      if (volume->v_flags & AFPVOL_TM) {
        if (volume->v_uuid) {
          ...
          strlist = avahi_string_list_add_printf(
              strlist, "dk%u=adVN=%s,adVF=0xa1,adVU=%s", i++, tmpname,
              volume->v_uuid);
        } else {
          LOG(log_warning, logtype_afpd,
              ...
              volume->v_localname);
          strlist = avahi_string_list_add_printf(
              strlist, "dk%u=adVN=%s,adVF=0xa1", i++, tmpname);
        }
      }
    }
    ...
    for (config = ctx->configs; config; config = config->next) {
      ...
      if (avahi_entry_group_add_service(ctx->group, AVAHI_IF_UNSPEC,
                                        AVAHI_PROTO_UNSPEC, 0, dsi->bonjourname,
                                        AFP_DNS_SERVICE_TYPE, NULL, NULL, port,
                                        NULL) < 0) {
      ...
      if (i &&
          avahi_entry_group_add_service_strlst(
              ctx->group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0,
              dsi->bonjourname, ADISK_SERVICE_TYPE, NULL, NULL, 9, /* discard */
              strlist) < 0) {
        LOG(log_error, logtype_afpd, "Failed to add service: %s",
            avahi_strerror(avahi_client_errno(ctx->client)));
        goto fail;
      ...
      if (config->obj.options.mimicmodel) {
        strlist2 = avahi_string_list_add_printf(strlist2, "model=%s",
                                                config->obj.options.mimicmodel);
        if (avahi_entry_group_add_service_strlst(
                ctx->group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0,
                dsi->bonjourname, DEV_INFO_SERVICE_TYPE, NULL, NULL, 0,
                strlist2) < 0) {
          LOG(log_error, logtype_afpd, "Failed to add service: %s",
              avahi_strerror(avahi_client_errno(ctx->client)));
          goto fail;
...
void av_zeroconf_register(const AFPConfig *configs) {
  ...
  if (!(ctx->client = avahi_client_new(
            avahi_threaded_poll_get(ctx->threaded_poll), AVAHI_CLIENT_NO_FAIL,
...
int av_zeroconf_unregister() {
  ...
  if (ctx) {
    LOG(log_error, logtype_afpd,
        "av_zeroconf_unregister: avahi_threaded_poll_stop");
    ...
    if (ctx->client)
      avahi_client_free(ctx->client);
    LOG(log_error, logtype_afpd,
        "av_zeroconf_unregister: avahi_threaded_poll_free");
    if (ctx->threaded_poll)
