      ...
      if (S_ISDIR(st.st_mode)) {
        if (AFP_OK != (err = copydir(vol, dirfd, spath, dpath)))
          goto copydir_done;
                 ...
                 (err = copyfile(vol, vol, dirfd, spath, dpath, NULL, NULL))) {
        goto copydir_done;
...
static int cname_mtouname(const struct vol *vol, struct dir *dir,
  ...
  if (ret->u_name == NULL) {
    if ((ret->u_name =
             mtoupath(vol, ret->m_name, dir->d_did, utf8_encoding())) == NULL) {
      afp_errno = AFPERR_PARAM;
      return -1;
    }
  }
...
int caseenumerate(const struct vol *vol, struct path *path, struct dir *dir) {
  ...
  if (NULL == (dp = opendir("."))) {
    LOG(log_debug, logtype_afpd, "caseenumerate: opendir failed: %s",
        dir->d_u_name);
    return -1;
  }

  /* LOG(log_debug, logtype_afpd, "caseenumerate: for %s", path->u_name); */
  if ((size_t)-1 == convert_string(vol->v_volcharset, CH_UCS2, path->u_name, -1,
                                   u2_path, sizeof(u2_path)))
    LOG(log_debug, logtype_afpd, "caseenumerate: conversion failed for %s",
        path->u_name);

  /*LOG(log_debug, logtype_afpd, "caseenumerate: dir: %s, path: %s",
   * dir->d_u_name, path->u_name); */
  ret = -1;
  for (de = readdir(dp); de != NULL; de = readdir(dp)) {
    if (NULL == check_dirent(vol, de->d_name))
      continue;

    if ((size_t)-1 == convert_string(vol->v_volcharset, CH_UCS2, de->d_name, -1,
                                     u2_dename, sizeof(u2_dename)))
      continue;
    ...
    if (strcasecmp_w(u2_path, u2_dename) == 0) {
      ...
      path->d_dir = NULL;
      if (of_stat(vol, path) == 0) {
        LOG(log_debug, logtype_afpd, "caseenumerate: using dir: %s, path: %s",
  ...
  /* LOG(log_debug, logtype_afpd, "caseenumerate: path on ret: %s",
   * path->u_name); */
  return ret;
}
...
struct dir *dir_new(const char *m_name, const char *u_name,
  ...
  if (convert_string_allocate(
          (utf8_encoding()) ? CH_UTF8_MAC : vol->v_maccharset, CH_UCS2, m_name,
...
int getdirparams(const struct vol *vol, u_int16_t bitmap, struct path *s_path,
    ...
    if (!ad_metadata(upath, ADFLAGS_CREATE | ADFLAGS_DIR, &ad)) {
      ...
      if (ad.ad_md->adf_flags & O_CREAT) {
        ...
        if (s_path->m_name == NULL) {
          if ((s_path->m_name =
                   utompath(vol, upath, dir->d_did, utf8_encoding())) == NULL) {
            LOG(log_error, logtype_afpd,
...
int setdirparams(struct vol *vol, struct path *path, u_int16_t d_bitmap,
  ...
  if (ad_open_metadata(upath, ADFLAGS_DIR, O_CREAT, &ad) < 0) {
    ...
    if (!vol_noadouble(vol) &&
        (d_bitmap & ~((1 << DIRPBIT_ACCESS) | (1 << DIRPBIT_UNIXPR) |
  ...
  while (bitmap != 0) {
    ...
    case DIRPBIT_FINFO:
      ...
      break;
    case DIRPBIT_UID: /* What kind of loser mounts as root? */
      if ((dir->d_did == DIRDID_ROOT) && (setdeskowner(ntohl(owner), -1) < 0)) {
 ...
 * to dirfd
 */
int renamedir(const struct vol *vol, int dirfd, char *src, char *dst,
...
int afp_mapname(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_, char *rbuf,
                size_t *rbuflen) {
  struct passwd *pw;
  struct group *gr;
  int len, sfunc;
  ...
  sfunc = (unsigned char)*ibuf++;
  *rbuflen = 0;
  LOG(log_debug, logtype_afpd, "afp_mapname: sfunc: %d, afp_version: %d", sfunc,
      afp_version);
  switch (sfunc) {
  case 1:
  case 2: /* unicode */
