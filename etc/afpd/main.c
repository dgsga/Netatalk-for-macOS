...
/* Two pointers to dynamic allocated arrays which store pollfds and associated
 * data */
...
static void fd_set_listening_sockets(void) {
  ...
  for (config = configs; config; config = config->next) {
    ...
    fdset_add_fd(default_options.connections + AFP_LISTENERS + FDSET_SAFETY,
                 &fdset, &polldata, &fdset_used, &fdset_size, config->fd,
                 LISTEN_FD, config);
  }

  if (default_options.flags & OPTION_KEEPSESSIONS)
    fdset_add_fd(default_options.connections + AFP_LISTENERS + FDSET_SAFETY,
                 &fdset, &polldata, &fdset_used, &fdset_size,
                 disasociated_ipc_fd, DISASOCIATED_IPC_FD, NULL);
}
...
int main(int ac, char **av) {
  ...
  if (!(server_children =
            server_child_alloc(default_options.connections, CHILD_NFORKS))) {
      ...
      if (fdset[i].revents & (POLLIN | POLLERR | POLLHUP)) {
        ...
        case IPC_FD:
          ...
          if (ipc_server_read(server_children, child->ipc_fd) != 0) {
            ...
            if ((default_options.flags & OPTION_KEEPSESSIONS) &&
                child->disasociated) {
              LOG(log_note, logtype_afpd, "main: removing reattached child[%u]",
                  child->pid);
        ...
        case DISASOCIATED_IPC_FD:
          ...
          if ((child = server_child_add(server_children, CHILD_DSIFORK, pid,
                                        recon_ipc_fd)) == NULL) {
