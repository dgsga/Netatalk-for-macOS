...
static void status_flags(char *data, const int notif, const int ipok,
  ...
  status = AFPSRVRINFO_COPY | AFPSRVRINFO_SRVSIGNATURE | AFPSRVRINFO_SRVMSGS |
           AFPSRVRINFO_FASTBOZO | AFPSRVRINFO_SRVRDIR | AFPSRVRINFO_SRVUTF8 |
...
static size_t status_directorynames(char *data, int *diroffset,
  ...
  if (options->k5service && options->k5realm && options->fqdn) {
    ...
    if (p)
      *p = '\0';
    len = strlen(options->k5service) + strlen(options->fqdn) +
    ...
    if (len > 255 || len + 2 > maxstatuslen - offset) {
      ...
      *data++ = len;
      snprintf(data, len + 1, "%s/%s@%s", options->k5service, options->fqdn,
               options->k5realm);
      data += len;
      if (p)
        *p = ':';
...
static size_t status_utf8servername(char *data, int *nameoffset,
  ...
  if ((size_t)-1 ==
      (len = convert_string(options->unixcharset, CH_UTF8_MAC, Obj, -1,
                            data + sizeof(namelen), maxstatuslen - offset))) {
...
void status_init(AFPConfig *aspconfig, AFPConfig *dsiconfig,
  ...
  status_flags(status, options->server_notif, (options->fqdn || ipok),
               options->passwdbits,
               (options->k5service && options->k5realm && options->fqdn),
               options->flags);
  /* returns offset to signature offset */
  c = status_server(
      status, options->server ? options->server : options->hostname, options);
...
void set_signature(struct afp_options *options) {
  ...
  if (strcmp(options->signatureopt, "auto") == 0) {
    ...
    LOG(log_warning, logtype_afpd,
        "WARNING: option \"-signature host\" is obsoleted. Switching back to "
        "auto.",
...
server_signature_user:
  ...
  if (len <= 5) {
    LOG(log_warning, logtype_afpd,
        "WARNING: option \"-signature %s\" is not valid. Switching back to "
        "auto.",
        options->signatureopt);
  ...
  if (len > 16) {
    LOG(log_warning, logtype_afpd,
        "WARNING: signature user string %s is very long !", usersign);
    len = 16;
  } else if (len >= 3) {
    LOG(log_info, logtype_afpd, "signature user string is %s.", usersign);
  } else {
    LOG(log_warning, logtype_afpd,
        "WARNING: signature user string %s is very short !", usersign);
  }
...
server_signature_random:
  ...
  if (fp && header) { /* conf file is created or size=0 */
    ...
    fprintf(fp, "# \n");
    fprintf(fp, "# If setting \"-signature user:xxxxx\" in afpd.conf, this "
                "file is not used.\n\n");
  }
...
server_signature_done:
  ...
  LOG(log_info, logtype_afpd,
      ...
      server_tmp, (options->signature)[0], (options->signature)[1],
      (options->signature)[2], (options->signature)[3], (options->signature)[4],
      (options->signature)[5], (options->signature)[6], (options->signature)[7],
      (options->signature)[8], (options->signature)[9],
      (options->signature)[10], (options->signature)[11],
      (options->signature)[12], (options->signature)[13],
      (options->signature)[14], (options->signature)[15]);
