...
static int catsearch(struct vol *vol, struct dir *dir, int rmatches,
                     uint32_t *pos, char *rbuf, uint32_t *nrecs, int *rsize,
                     int ext) {
  static u_int32_t cur_pos; /* Saved position index (ID) - used to remember
 ...
 * @param ext       (r)  extended search flag
 */
static int catsearch_db(struct vol *vol, struct dir *dir, const char *uname,
                        int rmatches, uint32_t *pos, char *rbuf,
                        uint32_t *nrecs, int *rsize, int ext) {
  ...
  if (cur_pos == 0 || *pos == 0) {
    ...
    if ((num_matches = cnid_find(vol->v_cdb, buffer, strlen(uname), resbuf,
                                 sizeof(resbuf))) == -1) {
      result = AFPERR_MISC;
      goto catsearch_end;
    }
  ...
  while (cur_pos < num_matches) {
    ...
    if ((name = cnid_resolve(vol->v_cdb, &did, resolvebuf,
...
static int catsearch_afp(AFPObj *obj _U_, char *ibuf, size_t ibuflen,
  ...
  *rbuflen = 24;
  if ((c1.rbitmap & (1 << FILPBIT_PDINFO)) &&
    ...
    ret = catsearch_db(vol, vol->v_root, uname, rmatches, &catpos[0], rbuf + 24,
                       &nrecs, &rsize, ext);
  else
    /* perform a slow filesystem tree search */
    ret = catsearch(vol, vol->v_root, rmatches, &catpos[0], rbuf + 24, &nrecs,
                    &rsize, ext);
