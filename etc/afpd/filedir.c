  ...
  *rbuflen = 0;
  ...
  if (S_ISDIR(st->st_mode)) {
    ...
    *(rbuf + 2 * sizeof(u_int16_t)) = (char)FILDIRBIT_ISDIR;
    ...
    if (fbitmap &&
        AFP_OK != (ret = getfilparams(vol, fbitmap, s_path, curdir,
                                      rbuf + 3 * sizeof(u_int16_t), &buflen))) {
      return (ret);
    }
    /* this is a file */
...
*/
static int moveandrename(const struct vol *vol, struct dir *sdir, int sdir_fd,
  ...
  if (!isdir) {
    if ((oldunixname = strdup(
             mtoupath(vol, oldname, sdir->d_did, utf8_encoding()))) == NULL)
  ...
  if (NULL ==
      (upath = mtoupath(vol, newname, curdir->d_did, utf8_encoding()))) {
    rc = AFPERR_PARAM;
    goto exit;
  }
  path.u_name = upath;
