...
void afp_options_free(struct afp_options *opt, const struct afp_options *save) {
...
int afp_options_parseline(char *buf, struct afp_options *options) {
  ...
  if ((c = getoption(buf, "-hostname"))) {
    ...
    if (len <= MAXHOSTNAMELEN) {
      ...
      LOG(log_info, logtype_afpd, "WARNING: hostname %s is too long (%d)", c,
          len);
  }
  ...
  c = buf;
  /* Now THIS is hokey! Multiple occurrences are not supported by our current
   * code, */
  /* so I have to loop myself. */
  while (NULL != (c = strstr(c, "-setuplog"))) {
    char *optstr;
    if ((optstr = getoption(c, "-setuplog"))) {
      /* hokey2: options->logconfig must be converted to store an array of
       * logstrings */
  ...
  if ((c = getoption(buf, "-k5keytab"))) {
    if (NULL ==
        (options->k5keytab = (char *)malloc(sizeof(char) * (strlen(c) + 14)))) {
  ...
  if ((c = getoption(buf, "-fqdn"))) {
    ...
    if (gethostbyname(c)) {
      ...
      LOG(log_error, logtype_afpd,
          "error parsing -fqdn, gethostbyname failed for: %s", c);
    }
  }
...
static void show_usage(char *name) {
  fprintf(stderr,
          "Usage:\t%s [-duptDTI] [-f defaultvolumes] [-s systemvolumes] [-n "
          "nbpname]\n",
          name);
  fprintf(stderr, "\t     [-c maxconnections] [-g guest] [-P pidfile] [-S "
                  "port] [-L message]\n");
