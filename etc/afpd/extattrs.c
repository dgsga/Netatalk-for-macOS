...
int afp_listextattr(AFPObj *obj _U_, char *ibuf, size_t ibuflen _U_, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  /*
    If its the first request with maxreply=0 or if we didn't mark our buffers
    valid for whatever reason (just a safety check, it should be valid), then
    scan for attributes
  */
    ...
    */
    ...
    if (adp) {
      ...
      if (memcmp(FinderInfo, emptyFinderInfo, 32) != 0) {
        /* FinderInfo contains some non 0 bytes -> include
      ...
      /* Now check for Ressource fork and add virtual EA
       * "com.apple.ResourceFork" if size > 0 */
      LOG(log_debug7, logtype_afpd,
