...
int dsi_tcp_init(DSI *dsi, const char *hostname, const char *address,
  ...
  if ((ret = getaddrinfo(address ? address : NULL, port ? port : "548", &hints,
                         &servinfo)) != 0) {
...
#ifdef SO_REUSEADDR
      flag = 1;
      setsockopt(dsi->serversock, SOL_SOCKET, SO_REUSEADDR, &flag,
                 sizeof(flag));
#endif
...
#define SOL_TCP IPPROTO_TCP
  ...
  if ((ret = getaddrinfo(hostname, port ? port : "548", &hints, &servinfo)) !=
      0) {
