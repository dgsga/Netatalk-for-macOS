...
*/
int stickydirmode(const char *name, const mode_t mode, const int dropbox,
                  const mode_t v_umask) {
  int retval = 0;
...
int setfilmode(const struct vol *vol, const char *name, mode_t mode,
  ...
  if (ochmod(name, mode & ~vol->v_umask, st,
    ...
    while (buflen > 0) {
      if ((cc = write(dfd, filebuf, buflen)) < 0) {
        ...
        LOG(log_error, logtype_afpd,
            "copy_file('%s'/'%s'): read '%s' error: %s", src, dst, dst,
            strerror(errno));
        ret = -1;
        goto exit;
      }
 ...
 *
 * unix_rename mulitplexes rename and renameat. If we dont HAVE_ATFUNCS, sfd and
 * dfd are ignored.
 *
 ...
 *
 * statat mulitplexes stat and fstatat. If we dont HAVE_ATFUNCS, dirfd is
 * ignored.
 *
