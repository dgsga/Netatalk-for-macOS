...
ssize_t sys_lgetxattr(const char *path, const char *uname, void *value,
  ...
  ssize_t retval;
  if ((retval = extattr_get_link(path, EXTATTR_NAMESPACE_USER, uname, NULL,
                                 0)) >= 0) {
    if (retval > size) {
      errno = ERANGE;
      return -1;
    }
    if ((retval = extattr_get_link(path, EXTATTR_NAMESPACE_USER, uname, value,
                                   size)) >= 0)
      return retval;
  }

  LOG(log_maxdebug, logtype_default,
      "sys_lgetxattr: extattr_get_link() failed with: %s\n", strerror(errno));
  return -1;
#elif defined(HAVE_ATTR_GET)
...
static ssize_t bsd_attr_list(int type, extattr_arg arg, char *list,
  ...
  case 2:
    list_size =
        extattr_list_fd(arg.filedes, EXTATTR_NAMESPACE_USER, list, size);
    break;
#endif
  default:
    errno = ENOSYS;
...
static ssize_t irix_attr_list(const char *path, int filedes, char *list,
  ...
  while (True) {
    if (filedes)
      retval =
          attr_listf(filedes, attr_buffer, ATTR_MAX_VALUELEN, flags, cursor);
    else
...
ssize_t sys_llistxattr(const char *path, char *list, size_t size) {
  ...
  ssize_t ret = -1;
  int attrdirfd =
      solaris_attropen(path, ".", O_RDONLY | AT_SYMLINK_NOFOLLOW, 0);
  if (attrdirfd >= 0) {
    ret = solaris_list_xattr(attrdirfd, list, size);
    close(attrdirfd);
  }
...
static int solaris_openat(int fildes, const char *path, int oflag,
  ...
  if (filedes == -1) {
    LOG(log_error, logtype_default, "openat(\"%s\"): %s", path,
        strerror(errno));
  }
  return filedes;
}
...
static int solaris_write_xattr(int attrfd, const char *value, size_t size) {
  if ((ftruncate(attrfd, 0) == 0) && (write(attrfd, value, size) == size)) {
    ...
    LOG(log_error, logtype_default, "solaris_write_xattr: %s", strerror(errno));
    return -1;
  }
}
