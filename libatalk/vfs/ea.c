...
static int unpack_header(struct ea *restrict ea) {
  ...
  if (*(uint32_t *)buf != htonl(EA_MAGIC)) {
    LOG(log_error, logtype_afpd, "unpack_header: wrong magic 0x%08x",
        *(uint32_t *)buf);
    ret = -1;
    goto exit;
  }
  ...
  if (*(uint16_t *)buf != htons(EA_VERSION)) {
    LOG(log_error, logtype_afpd, "unpack_header: wrong version 0x%04x",
        *(uint16_t *)buf);
    ret = -1;
    goto exit;
  }
...
int ea_chmod_dir(VFS_FUNC_ARGS_SETDIRUNIXMODE) {
  ...
  uid = geteuid();
  if (seteuid(0)) {
    LOG(log_error, logtype_afpd, "ea_chmod_dir('%s'): seteuid: %s", name,
        strerror(errno));
    return AFPERR_MISC;
  }
