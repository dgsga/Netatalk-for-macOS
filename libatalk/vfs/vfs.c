...
static int netatalk_name(const char *name) {
  return strcasecmp(name, ".AppleDouble") && strcasecmp(name, ".AppleDB") &&
...
static int RF_setdirunixmode_adouble(VFS_FUNC_ARGS_SETDIRUNIXMODE) {
  ...
  if (dir_rx_set(mode)) {
    if (stickydirmode(ad_dir(adouble), DIRBITS | mode, dropbox, vol->v_umask) <
        0)
      return -1;
  }
...
static int setdirowner_adouble_loop(const struct vol *vol,
  ...
  if (chown(name, owner->uid, owner->gid) < 0 && errno != EPERM) {
    LOG(log_debug, logtype_afpd, "setdirowner: chown %d/%d %s: %s", owner->uid,
...
static int RF_setdirowner_adouble(VFS_FUNC_ARGS_SETDIROWNER) {
  ...
  if (for_each_adouble("setdirowner", adouble_p, setdirowner_adouble_loop, vol,
                       &owner, noadouble))
    return -1;
...
static int RF_posix_remove_acl(VFS_FUNC_ARGS_REMOVE_ACL) {
  ...
  if (dir) {
    ...
    if (len < 0 || len >= MAXPATHLEN)
      return AFPERR_MISC;
    /* remove ACL from .AppleDouble/.Parent first */
    EC_ZERO_LOG_ERR(remove_acl_vfs(vol->ad_path(path, ADFLAGS_DIR)),
                    AFPERR_MISC);
    ...
    /* remove ACL from ressource fork */
    EC_ZERO_LOG_ERR(remove_acl_vfs(vol->ad_path(path, ADFLAGS_HF)),
                    AFPERR_MISC);
  }
...
static int ads_delete_rf(char *name) {
  ...
  if ((err = for_each_adouble("deletecurdir", name, deletecurdir_ads1_loop,
                              NULL, NULL, 1)))
    return err;
  /* FIXME
...
static int ads_setfilmode_loop(const struct vol *vol, struct dirent *de _U_,
                               char *name, void *data, int flag _U_) {
...
static int ads_setfilmode(const struct vol *vol, const char *name, mode_t mode,
                          struct stat *st) {
  ...
  if (for_each_adouble("setfilmode_ads", name, ads_setfilmode_loop, vol, &param,
                       0) < 0)
    return -1;
...
static int RF_setdirunixmode_ads(VFS_FUNC_ARGS_SETDIRUNIXMODE) {
  ...
  if (ads_setfilmode(vol, ad_dir(vol->ad_path(name, ADFLAGS_DIR)), mode, st) <
      0)
    return -1;
...
static int setdirmode_ads_loop(const struct vol *vol, struct dirent *de _U_,
                               char *name, void *data, int flag) {
  ...
  if (dir_rx_set(param->mode)) {
    if (stickydirmode(name, DIRBITS | param->mode, param->dropbox,
                      vol->v_umask) < 0) {
      if (flag) {
        return 0;
      }
...
static int RF_setdirmode_ads(VFS_FUNC_ARGS_SETDIRMODE) {
  ...
  if (dir_rx_set(mode)) {
    /* .AppleDouble */
    if (stickydirmode(ad_dir(ad_p), DIRBITS | mode, param.dropbox,
                      vol->v_umask) < 0)
      return -1;
  }

  if (for_each_adouble("setdirmode_ads", ad_dir(ad_p), setdirmode_ads_loop, vol,
...
static int setdirowner_ads1_loop(const struct vol *vol _U_,
                                 struct dirent *de _U_, char *name, void *data,
                                 int flag _U_) {
  struct perm *owner = data;
...
static int setdirowner_ads_loop(const struct vol *vol, struct dirent *de _U_,
                                char *name, void *data, int flag) {
  struct perm *owner = data;

  if (for_each_adouble("setdirowner", name, setdirowner_ads1_loop, vol, data,
                       flag) < 0)
    return -1;
...
static int RF_setdirowner_ads(VFS_FUNC_ARGS_SETDIROWNER) {
  ...
  if (for_each_adouble("setdirowner", ad_dir(adouble_p), setdirowner_ads_loop,
                       vol, &owner, noadouble))
    return -1;
...
static int validupath_osx(VFS_FUNC_ARGS_VALIDUPATH) {
  return strncmp(name, "._", 2) &&
         ((vol->v_flags & AFPVOL_USEDOTS) ? netatalk_name(name)
                                          : name[0] != '.');
}
...
/* ---------------- */
static int RF_setdirmode_osx(VFS_FUNC_ARGS_SETDIRMODE) { return 0; }

/* ---------------- */
static int RF_setdirowner_osx(VFS_FUNC_ARGS_SETDIROWNER) { return 0; }
