...
static int init_tsock(CNID_private *db) {
  ...
  LOG(log_debug, logtype_cnid,
      "init_tsock: BEGIN. Opening volume '%s', CNID Server: %s/%s", db->db_dir,
      db->cnidserver, db->cnidport);
...
static int send_packet(CNID_private *db, struct cnid_dbd_rqst *rqst) {
  ...
  if (write_vec(db->fd, iov, towrite, vecs) != towrite) {
    LOG(log_warning, logtype_cnid,
        "send_packet: Error writev rqst (db_dir %s): %s", db->db_dir,
        strerror(errno));
    return -1;
  }
...
static int dbd_rpc(CNID_private *db, struct cnid_dbd_rqst *rqst,
  ...
  if (ret != sizeof(struct cnid_dbd_rply)) {
    LOG(log_debug, logtype_cnid,
        "dbd_rpc: Error reading header from fd (db_dir %s): %s", db->db_dir,
        ret == -1 ? strerror(errno) : "closed");
    rply->name = nametmp;
    return -1;
  }
  rply->name = nametmp;
  if (rply->namelen && rply->namelen > len) {
    LOG(log_error, logtype_cnid,
        "dbd_rpc: Error reading name (db_dir %s): %s name too long: %d. only "
        "wanted %d, garbage?",
  ...
  if (rply->namelen && (ret = readt(db->fd, rply->name, rply->namelen, 0,
    ...
    LOG(log_error, logtype_cnid,
        "dbd_rpc: Error reading name from fd (db_dir %s): %s", db->db_dir,
        ret == -1 ? strerror(errno) : "closed");
    return -1;
  }
...
static int transmit(CNID_private *db, struct cnid_dbd_rqst *rqst,
  ...
  while (1) {
    if (db->fd == -1) {
      ...
      LOG(log_debug, logtype_cnid,
          "transmit: attached to '%s', stamp: '%08lx'.", db->db_dir,
          *(uint64_t *)stamp);
    }
  ...
  transmit_fail:
    ...
    if (errno == ECONNREFUSED) { /* errno carefully injected in tsock_getfd */
      /* give up */
      LOG(log_error, logtype_cnid, "transmit: connection refused (db_dir %s)",
          db->db_dir);
      return -1;
    }
...
static struct _cnid_db *cnid_dbd_new(const char *volpath) {
  ...
  cdb->cnid_add =
      (cnid_t(*)(struct _cnid_db *, const struct stat *, const cnid_t,
                 const char *, const size_t, cnid_t))cnid_dbd_add;
  cdb->cnid_delete = cnid_dbd_delete;
  cdb->cnid_get = cnid_dbd_get;
  cdb->cnid_lookup = cnid_dbd_lookup;
  cdb->cnid_find = (int (*)(struct _cnid_db *, const char *, size_t, void *,
                            size_t))cnid_dbd_find;
...
cnid_t cnid_dbd_add(struct _cnid_db *cdb, const struct stat *st,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_dbd_add: CNID: %u, name: '%s', inode: 0x%llx, type: %d (0=file, "
      "1=dir)",
