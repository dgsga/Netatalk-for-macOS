...
static int ipc_kill_token(struct ipc_header *ipc, server_child *children) {
  ...
  return server_child_transfer_session(children, CHILD_DSIFORK, pid, ipc->uid,
                                       ipc->afp_socket, ipc->DSI_requestID);
}
...
static int ipc_get_session(struct ipc_header *ipc, server_child *children) {
  ...
  server_child_kill_one_by_id(children, CHILD_DSIFORK, ipc->child_pid, ipc->uid,
                              idlen, clientid, boottime);
...
int ipc_server_uds(const char *name) {
  ...
  address_length =
      sizeof(address.sun_family) + sprintf(address.sun_path, "%s", name);
