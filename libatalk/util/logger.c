...
#define LOGLEVEL_STRING_IDENTIFIERS                                            \
  {                                                                            \
    "LOG_NOTHING", "LOG_SEVERE", "LOG_ERROR", "LOG_WARN", "LOG_NOTE",          \
...
#define LOGTYPE_STRING_IDENTIFIERS                                             \
  {                                                                            \
    "Default", "Logger", "CNID", "AFPDaemon", "DSI", "ATalkDaemon",            \
        "PAPDaemon", "UAMS", "end_of_list_marker"                              \
  }
...
/* We use this in order to track the last n log messages in order to prevent
 * flooding */
#define LOG_FLOODING_MINCOUNT                                                  \
  5 /* this controls after how many consecutive messages must be detected      \
       before we start to hide them */
#define LOG_FLOODING_MAXCOUNT                                                  \
  1000 /* this controls after how many consecutive messages we force a         \
          "repeated x times" message */
#define LOG_FLOODING_ARRAY_SIZE                                                \
  3 /* this contols how many messages in flow we track */
...
static void generate_message_details(char *message_details_buffer,
  ...
  *ptr = 0;
  ...
  if (!(display_options & logoption_nsrcinfo)) {
    ...
    if (basename)
      templen =
          snprintf(ptr, len, " {%s:%d}", basename + 1, log_src_linenumber);
    else
      templen =
          snprintf(ptr, len, " {%s:%d}", log_src_filename, log_src_linenumber);
    if (templen == -1 || templen >= len)
      return;
    len -= templen;
    ptr += templen;
  ...
  if (strcmp(filename, "/dev/tty") == 0) {
    ...
    if (process_uid) {
      if (seteuid(process_uid) == -1) {
        LOG(log_error, logtype_logger, "can't seteuid back %s",
            strerror(errno));
        exit(EXITERR_SYS);
      }
    }
...
void make_log_entry(enum loglevels loglevel, enum logtypes logtype,
  ...
  /* Prevent flooding: hash the message and check if we got the same one
   * recently */
  ...
  if (log_flood_entries == LOG_FLOODING_ARRAY_SIZE) {
    /* array is full, discard oldest entry printing "message repeated..." if
     * count > 1 */
...
log:
  if (!log_config.console) {
    generate_message_details(
        log_details_buffer, sizeof(log_details_buffer),
