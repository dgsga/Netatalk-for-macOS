...
static const vol_opt_name_t vol_opt_names[] = {
    ...
    {AFPVOL_MSWINDOWS,
     "MSWINDOWS"}, /* deal with ms-windows yuckiness. this is going away. */
    ...
    {AFPVOL_NOFILEID,
     "NOFILEID"}, /* don't advertise createid resolveid and deleteid calls */
    {AFPVOL_NOSTAT,
     "NOSTAT"}, /* advertise the volume even if we can't stat() it
    ...
    {AFPVOL_CACHE,
     "CACHEID"}, /* Use adouble v2 CNID caching, default don't use it */
...
static const info_option_t info_options[] = {{"MAC_CHARSET", MAC_CHARSET},
...
int vol_load_charsets(struct volinfo *vol) {
  ...
  if ((charset_t)-1 == (vol->v_volcharset = add_charset(vol->v_volcodepage))) {
    fprintf(stderr, "Setting codepage %s as volume codepage failed",
            vol->v_volcodepage);
    return (-1);
  }
...
int loadvolinfo(char *path, struct volinfo *vol) {
  ...
  if (NULL == (fp = fopen(volinfofile, "r"))) {
    fprintf(stderr, "error opening volinfo (%s): %s", volinfofile,
            strerror(errno));
    return (-1);
  }
  fd = fileno(fp);
...
/*!
 * Allocate a struct volinfo object for refcounting usage with retain and close,
 * and call loadvolinfo with it
 */
...
void retainvolinfo(struct volinfo *vol) { vol->retaincount++; }
...
int savevolinfo(const struct vol *vol, const char *Cnid_srv,
  ...
  if ((fd = open(item, O_RDWR | O_CREAT, 0666)) < 0) {
    LOG(log_debug, logtype_default, "Error opening %s: %s", item,
        strerror(errno));
    if (process_uid) {
      if (seteuid(process_uid) == -1) {
        LOG(log_error, logtype_default, "can't seteuid back %s",
            strerror(errno));
        exit(EXITERR_SYS);
      }
    }
  ...
  if (fcntl(fd, F_SETLK, &lock) < 0) {
    ...
    if (errno == EACCES || errno == EAGAIN) {
      ...
      LOG(log_error, logtype_default, "savevoloptions: cannot get lock: %s",
          strerror(errno));
      return (-1);
    }
  }
  ...
  if (strlen(buf) >= sizeof(buf) - 1)
    LOG(log_debug, logtype_default,
        "Error writing .volinfo file: buffer too small, %s", buf);
  if (write(fd, buf, strlen(buf)) < 0 || ftruncate(fd, strlen(buf)) < 0) {
    LOG(log_debug, logtype_default, "Error writing .volinfo file: %s",
        strerror(errno));
  }
