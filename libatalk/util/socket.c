 ...
 * @param data        (rw) pointer to data the caller want to associate with an
 * fd
 */
void fdset_add_fd(int maxconns, struct pollfd **fdsetp,
                  struct polldata **polldatap, int *fdset_usedp,
  ...
  LOG(log_debug, logtype_default, "fdset_add_fd: adding fd %i in slot %i", fd,
      *fdset_usedp);
  ...
  if (fdset == NULL) { /* 1 */
    ...
    *polldatap = polldata;
    ...
    LOG(log_debug, logtype_default,
        "fdset_add_fd: initialized with space for %i conncections", maxconns);
  }
 ...
 */
void fdset_del_fd(struct pollfd **fdsetp, struct polldata **polldatap,
  ...
  for (int i = 0; i < *fdset_usedp; i++) {
    if (fdset[i].fd == fd) {         /* 1 */
      if ((i + 1) == *fdset_usedp) { /* 2a */
        ...
        memmove(&fdset[i], &fdset[i + 1],
                (*fdset_usedp - i - 1) * sizeof(struct pollfd)); /* 3 */
        memmove(&polldata[i], &polldata[i + 1],
