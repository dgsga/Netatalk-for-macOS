...
int islower_sp(u_int32_t c_sp) { return (c_sp == tolower_sp(c_sp)); }
...
int isupper_sp(u_int32_t c_sp) { return (c_sp == toupper_sp(c_sp)); }
...
ucs2_t *strcasechr_sp(const ucs2_t *s, u_int32_t c_sp) {
  if (*s == 0)
    return NULL;
  while (s[1] != 0) {
    if (tolower_sp(c_sp) == tolower_sp((u_int32_t)*s << 16 | (u_int32_t)s[1]))
      return (ucs2_t *)s;
    s++;
  }
...
int strcasecmp_w(const ucs2_t *a, const ucs2_t *b) {
  ...
  while (*a && *b) {
    if ((0xD800 <= *a) && (*a < 0xDC00)) {
      if ((ret = tolower_sp((u_int32_t)*a << 16 | (u_int32_t)a[1]) -
                 tolower_sp((u_int32_t)*b << 16 | (u_int32_t)b[1])))
        return ret;
      a++;
      b++;
      if (!(*a && *b))
...
static u_int32_t do_precomposition_sp(unsigned int base_sp,
  ...
  int mid;
  u_int64_t sought_sp = ((u_int64_t)base_sp << 32) | (u_int64_t)comb_sp,
            that_sp;

  /* binary search */
  while (max >= min) {
    mid = (min + max) / 2;
    that_sp = ((u_int64_t)precompositions_sp[mid].base_sp << 32) |
              ((u_int64_t)precompositions_sp[mid].comb_sp);
...
static u_int64_t do_decomposition_sp(unsigned int base_sp) {
  ...
  while (max >= min) {
    ...
    if (that_sp < sought_sp) {
      ...
      result_sp = ((u_int64_t)decompositions_sp[mid].base_sp << 32) |
                  ((u_int64_t)decompositions_sp[mid].comb_sp);
      return result_sp;
    }
  }
  /* no match */
