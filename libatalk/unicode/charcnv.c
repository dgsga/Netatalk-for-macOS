    ...
    if (ln) {
      ...
      if (handle == (atalk_iconv_t)-1) {
        LOG(log_debug, logtype_default,
            "Locale charset '%s' unsupported, using ASCII instead", ln);
        ln = "ASCII";
      } else {
...
static size_t pull_charset_flags(charset_t from_set, charset_t cap_set,
                                 const char *src, size_t srclen, char *dest,
...
static size_t push_charset_flags(charset_t to_set, charset_t cap_set, char *src,
  ...
  if ((option & CONV_ESCAPEDOTS) && i_len >= 2 &&
      SVAL(inbuf, 0) == 0x002e) { /* 0x002e = . */
    if (o_len < 3) {
      errno = E2BIG;
      goto end;
    }
...
size_t convert_charset(charset_t from_set, charset_t to_set,
                       ...
                       char *dest, size_t dest_len, u_int16_t *flags) {
  size_t i_len, o_len;
  ucs2_t *u;
  ...
  /* convert from_set to UCS2 */
  if ((size_t)(-1) ==
      (o_len = pull_charset_flags(from_set, cap_charset, src, src_len,
