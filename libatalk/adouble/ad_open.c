...
static int ad_update(struct adouble *ad, const char *path) {
  ...
  if (st.st_size > 0x7fffffff) {
    LOG(log_debug, logtype_default, "ad_update: file '%s' too big for update.",
        path);
    errno = EIO;
    goto bail_lock;
  }
  ...
  if (off > st.st_size) {
    LOG(log_error, logtype_default,
        "ad_update: invalid resource fork offset. (off: %u)", off);
    errno = EIO;
    goto bail_lock;
  }
  ...
  if ((void *)(buf = (char *)mmap(NULL, st.st_size + shiftdata,
  ...
  while (eid->id) {
    if (ad->ad_eid[eid->id].ade_off != 0) {
      if (eid->id > 2 && ad->ad_eid[eid->id].ade_len < 256)
        memcpy(databuf[eid->id], ad->ad_data + ad->ad_eid[eid->id].ade_off,
...
static int ad_convert(struct adouble *ad, const char *path) {
  ...
  if (off > st.st_size) {
    LOG(log_error, logtype_default,
        "ad_v1tov2: invalid resource fork offset. (off: %u)", off);
    errno = EIO;
    goto bail_lock;
  }
...
/* -------------------------------------
   read in the entries
*/
static void parse_entries(struct adouble *ad, char *buf, u_int16_t nentries) {
...
/* -------------------- */
static int ad_mkrf_osx(char *path _U_) { return 0; }
...
int ad_mkdir(const char *path, int mode) {
  ...
  LOG(log_debug, logtype_default, "ad_mkdir(\"%s\", %04o) {cwd: \"%s\"}", path,
      mode, getcwdpath());
...
static struct adouble_fops ad_osx = {
    &ad_path_osx,    &ad_mkrf_osx,       &ad_rebuild_adouble_header,
    &ad_check_size,
...
static struct adouble_fops ad_sfm = {
    &ad_path_sfm,           &ad_mkrf_sfm,
    &ad_rebuild_sfm_header, &ad_check_size_sfm,

    &ad_header_sfm_read,    &ad_header_upgrade_none,
};

static struct adouble_fops ad_adouble = {
    &ad_path,        &ad_mkrf,           &ad_rebuild_adouble_header,
    &ad_check_size,
 ...
 *                ADFLAGS_NOHF: it's not an error if header file couldn't be
 * created\n ADFLAGS_DIR: if path is a directory you MUST or ADFLAGS_DIR to
...
int ad_open(const char *path, int adflags, int oflags, int mode,
  ...
  if ((adflags & ADFLAGS_DF)) {
    if (ad_data_fileno(ad) == -1) {
      ...
      ad->ad_data_fork.adf_fd =
          open(path, hoflags | ad_get_syml_opt(ad), admode);
      ...
      if ((oflags & (O_RDWR | O_WRONLY)) && /* we want write access */
          !(ad->ad_data_fork.adf_flags &
            (O_RDWR | O_WRONLY))) /* and it was denied the first time */
      {
        errno = EACCES;
        return -1;
      }
  ...
  if (ad->ad_md->adf_fd < 0) {
    if (errno == ENOENT && (oflags & O_CREAT)) {
       ...
       */
      LOG(log_debug, logtype_default,
          "ad_open(\"%s\"): {cwd: \"%s\"} creating adouble file", ad_p,
          getcwdpath());
      admode = mode;
      errno = 0;
 ...
 *              ADFLAGS_CREATE: force creation of header file, but only as user,
 * not as root\n ADFLAGS_OPENFORKS: test if name is open by another afpd process
 *
 * @param adp   pointer to struct adouble
 *
 * @note caller MUST pass ADFLAGS_DIR for directories. Whether ADFLAGS_CREATE
 * really creates a adouble file depends on various other volume options, eg.
 * ADVOL_CACHE
 */
int ad_metadata(const char *name, int flags, struct adouble *adp) {
  ...
  if ((adp->ad_options & ADVOL_CACHE) && !(adp->ad_options & ADVOL_NOADOUBLE) &&
  ...
  if ((ret = ad_open(name, ADFLAGS_HF | dir, create, 0666, adp)) < 0 &&
    ...
    uid = geteuid();
    if (seteuid(0)) {
      LOG(log_error, logtype_default, "ad_metadata(%s): seteuid failed %s",
          name, strerror(errno));
      errno = EACCES;
      return -1;
    }
...
int ad_openat(int dirfd, /* dir fd openat like */
              const char *path, int adflags, int oflags, int mode,
              struct adouble *ad) {
  int ret = 0;
  int cwdfd = -1;
