...
int ad_setattr(const struct adouble *ad, const u_int16_t attribute) {
  ...
  if (ad->ad_version == AD_VERSION1) {
    if (ad_getentryoff(ad, ADEID_FILEI)) {
      memcpy(ad_entry(ad, ADEID_FILEI) + FILEIOFF_ATTR, &attr, sizeof(attr));
    }
  }
#if AD_VERSION == AD_VERSION2
  else if (ad->ad_version == AD_VERSION2) {
...
/* ----------------------------- */
u_int32_t ad_getid(struct adouble *adp, const dev_t st_dev, const ino_t st_ino,
  ...
  if (adp && (adp->ad_options & ADVOL_CACHE) &&
      ...
      (sizeof(dev_t) ==
       ad_getentrylen(
           adp, ADEID_PRIVDEV)) /* One check to ensure ALL values are there */
  ) {
    ...
    if (((adp->ad_options & ADVOL_NODEV) || dev == st_dev) && ino == st_ino &&
        (!did || a_did == did) && (memcmp(stamp, temp, sizeof(temp)) == 0)) {
