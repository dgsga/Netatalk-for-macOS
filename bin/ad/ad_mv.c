...
static char *netatalk_dirs[] = {".AppleDouble", ".AppleDB", ".AppleDesktop",
                                NULL};
...
static int do_move(const char *from, const char *to) {
   ...
   */
  if (!svolume.volinfo.v_path || !dvolume.volinfo.v_path ||
  ...
  if (!mustcopy) {
    ...
    case S_IFREG:
      if (dvolume.volume.vfs->vfs_renamefile(&dvolume.volume, -1, from, to) !=
          0) {
    ...
    if (cnid_update(dvolume.volume.v_cdb, cnid, &sb, newdid, name,
                    strlen(name)) != 0) {
    ...
    if (ad_open_metadata(to, S_ISDIR(sb.st_mode) ? ADFLAGS_DIR : 0, O_RDWR,
                         &ad) != 0) {
