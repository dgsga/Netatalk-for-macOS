...
int ad_find(int argc, char **argv) {
  ...
  if (convert_charset(vol.volinfo.v_volcharset, vol.volinfo.v_volcharset,
                      vol.volinfo.v_maccharset, argv[optind],
                      strlen(argv[optind]), namebuf, MAXPATHLEN,
                      &flags) == (size_t)-1) {
  ...
  if ((count = cnid_find(vol.volume.v_cdb, namebuf, strlen(namebuf), resbuf,
                         sizeof(resbuf))) < 1) {
    ret = 1;
  } else {
    ret = 0;
    ...
    while (count--) {
      ...
      while (did != DIRDID_ROOT) {
        if ((name = cnid_resolve(vol.volume.v_cdb, &did, buffer, buflen)) ==
            NULL)
          goto next;
