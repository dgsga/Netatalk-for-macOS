...
static char *mtoupathsjis(char *from) {
    ...
    if (*in) {
        ...
        while (out < maxout) {
            ...
            } else if ((0xA1 <= p) && (p <= 0xDF)) { ;    /* halfwidth katakana */
            } else if (p < 0x80) {
#ifdef DOWNCASE
                p = ( isupper( p )) ? tolower( p ) : p;
#endif /* DOWNCASE */
...
static char *utompathiconv(char *upath) {
    ...
    /* convert charsets */
    if ((size_t) -1 ==
        (outlen = convert_charset(vol.v_volcharset, vol.v_maccharset, vol.v_maccharset, u, outlen, mpath, MAXPATHLEN,
                                  &flags))) {
...
static char *mtoupathiconv(char *mpath) {
    ...
    if ((size_t) -1 ==
        (outlen = convert_charset(vol.v_maccharset, vol.v_volcharset, vol.v_maccharset, m, inplen, u, outlen,
                                  &flags))) {
