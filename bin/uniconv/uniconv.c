...
static char *convert_name(char *name, struct stat *st, cnid_t cur_did) {
    ...
    outlen = convert_charset(ch_from, ch_to, ch_mac, (char *) p, strlen((char *) p), (char *) q, sizeof(buffer) - 2,
                             &flags);
    if ((size_t) -1 == outlen) {
        if (ch_to == CH_UTF8) {
            ...
            q = (unsigned char *) buffer;
            p = (unsigned char *) name;
            outlen = convert_charset(ch_to, ch_to, ch_mac, (char *) p, strlen((char *) p), (char *) q,
